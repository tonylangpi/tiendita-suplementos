...
function SaboresPage(){
  ...
  const { data, mutate} = useSWR(
    `${process.env.NEXT_PUBLIC_API_URL}sabores/all`,{
  ...
  const handleSaveRowEdits = async ({ exitEditingMode, row, values }) => {
    ...
    let res = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}sabores/updateSabor`, values);
  ...
  const columns = useMemo(
    ...
    () => [
      {
        accessorKey: "idSabor", //simple recommended way to define a column
        header: "ID",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}sabores/createSabor`, {
  ...
  return (
    ...
    <Toaster position="top-center" offset="80px"/>
      ...
      >
        <Modal.Header>Agregar Sabor</Modal.Header>
        <Modal.Body>
          <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
            <div>
              <div className="mb-2 block">
                <Label htmlFor="sabor" value="sabor" />
              </div>
              <TextInput id="sabor" required type="text" name="sabor" />
            </div>
          ...
          <div className="flex">
            <Button
              ...
              onClick={async () => {
                if (
                  !confirm(
                    `Deseas eliminar el sabor: ${row.getValue("sabor")}`
                  )
                ) {
                  return;
                }
                let res = await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}sabores/deleteSabor/${row.getValue(
                    "idSabor"
                  )}`
                );
