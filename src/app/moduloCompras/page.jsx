...
export default function Compras() {
  ...
  const { data, mutate } = useSWR(
    `${process.env.NEXT_PUBLIC_API_URL}encabezadosCompra/all`,
    {
      ...
      <div className="flex flex-col pt-5">
        <Card className="max-w-sm mb-4 md:max-w-2xl">
          ...
          <form className="grid grid-cols-2 gap-4" onSubmit={onSubmit}>
            <div>
              ...
              {errors.Tipo_Compra && (
                  <span className="text-red-600">{errors.Tipo_Compra.message}</span>
                )}
        ...
        <Card className="max-w-sm mb-4 md:max-w-2xl">
          <MaterialReactTable
            ...
            enableDensityToggle={false}
            initialState={{ density: "compact", columnVisibility:{Tipo_Compra:false, ProveedorId:false} }}
            muiTableProps={{
              sx: {
                border: "1px solid rgba(81, 81, 81, 1)",
              },
            ...
            renderRowActions={({ row, table }) => (
              <div className="flex">
              <Tooltip content="Agregar Detalles">
                <Button
                  color="failure"
                  pill
                  onClick={() => {
                     toast("agregar detalles");
                  }}
                >
