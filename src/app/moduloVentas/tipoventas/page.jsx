...
function TipoVentas(){
  ...
  /*LLAMADO A LA API PARA LA PETICION DE tipos de venta */
  const { data, mutate} = useSWR(
    `${process.env.NEXT_PUBLIC_API_URL}tipoventa/all`,{
  ...
  const handleSaveRowEdits = async ({ exitEditingMode, row, values }) => {
    ...
    let res = await axios.put(
      `${process.env.NEXT_PUBLIC_API_URL}tipoventa/updatetipoVenta`,
      values
    );
  ...
  const columns = useMemo(
    ...
    () => [
      {
        accessorKey: "idTipoVenta", //simple recommended way to define a column
        header: "ID",
      ...
      },
      {
        ...
        Header: <i style={{ color: "blue" }}>Tipo de venta</i>, //optional custom markup
      },
      {
  ...
  const save = handleSubmit(async (data) => {
    try {
      const res = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}tipoventa/create`,
        data
      );
  ...
  return (
    <>
      ...
      >
        <Modal.Header>Agregar tipo Venta</Modal.Header>
        <Modal.Body>
          <form className="flex flex-col gap-4" onSubmit={save}>
            <div>
              <div className="mb-2 block">
                <Label htmlFor="nombreTipoVenta" value="tipo de Venta" />
              </div>
              <TextInput
                id="nombreTipoVenta"
                type="text"
                name="nombreTipoVenta"
                {...register("nombreTipoVenta", {
                  required: {
                    value: true,
                    message: "nombre de tipo de venta  requerido",
                  },
                  maxLength: 50,
                  minLength: 2,
                })}
              />
              {errors.nombreTipoVenta && (
                <span className="text-red-600">{errors.nombreTipoVenta.message}</span>
              )}
              {errors.nombreTipoVenta?.type === "maxLength" && (
                <span className="text-red-600">
                  El nombre del tipo de venta no debe superar los 50 caracteres
                </span>
              )}
              {errors.nombreTipoVenta?.type === "minLength" && (
                <span className="text-red-600">
                  el nombre del tipo de venta debe ser mayor a 2 caracteres
                </span>
              )}
            </div>
      ...
      <h1>Tipos de ventas listados</h1>
      {
        ...
        renderRowActions={({ row, table }) => (
          ...
          <Button
              ...
              onClick={async () => {
                if (
                  !confirm(`Deseas cambiar el estado del tipo de venta: ${row.getValue("nombreTipoVenta")}`)
                ) {
                  return;
                }
                let res = await axios.delete(
                  ...
                  }tipoventa/deletetipoVenta/${row.getValue("idTipoVenta")}`
                );
